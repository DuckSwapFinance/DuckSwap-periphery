...
contract LimitOrderWithSwapManager is Switch, Base, IDuckSwapAddLimOrderCallback, IDuckSwapCallback {
    ...
    // callback data passed through DuckSwapPool#addLimOrderWithX(Y) to the callback
    ...
    /// @param factory address of DuckSwapFactory
    ...
    function getEarnX(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = IDuckSwapPool(pool).userEarnX(key);
    ...
    function getEarnY(address pool, bytes32 key) private view returns(uint256, uint128, uint128) {
        (uint256 lastAccEarn, , , uint128 earn, uint128 legacyEarn, ) = IDuckSwapPool(pool).userEarnY(key);
        return (lastAccEarn, earn, legacyEarn);
    }
    ...
    function _addLimOrder(
        ...
        if (addLimitOrderParam.sellXEarnY) {
            (order, acquire) = IDuckSwapPool(pool).addLimOrderWithX(
            ...
            (order, acquire) = IDuckSwapPool(pool).addLimOrderWithY(
                address(this), addLimitOrderParam.pt, addLimitOrderParam.amount,
                abi.encode(LimCallbackData({tokenX: addLimitOrderParam.tokenX, tokenY: addLimitOrderParam.tokenY, fee: addLimitOrderParam.fee, payer: msg.sender}))
            );
        }
        if (acquire > 0) {
            IDuckSwapPool(pool).collectLimOrder(addLimitOrderParam.recipient, addLimitOrderParam.pt, 0, acquire, addLimitOrderParam.sellXEarnY);
        }
    }
        ...
        if (addLimitOrderParam.sellXEarnY) {
            if (addLimitOrderParam.pt < currentPoint) {
                ...
                if (addLimitOrderParam.isDesireMode) {
                    (costX, acquireY) = IDuckSwapPool(pool).swapX2YDesireY(
                    ...
                    (costX, acquireY) = IDuckSwapPool(pool).swapX2Y(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX, 
                            fee: addLimitOrderParam.fee, 
            ...
            if (addLimitOrderParam.pt > currentPoint) {
                ...
                uint256 acquireX;
                if (addLimitOrderParam.isDesireMode) {
                    (acquireX, costY) = IDuckSwapPool(pool).swapY2XDesireX(
                    ...
                    (acquireX, costY) = IDuckSwapPool(pool).swapY2X(
                        recipient, addLimitOrderParam.amount, addLimitOrderParam.pt + 1,
                        abi.encode(SwapCallbackData({
                            tokenX: addLimitOrderParam.tokenX, 
                            fee: addLimitOrderParam.fee, 
    ...
    function newLimOrder(
        ...
        address pool = IDuckSwapFactory(factory).pool(addLimitOrderParam.tokenX, addLimitOrderParam.tokenY, addLimitOrderParam.fee);
    ...
    function assignLimOrderEarn(
        ...
        if (isEarnY) {
            actualAssign = IDuckSwapPool(pool).assignLimOrderEarnY(pt, amount, fromLegacy);
        } else {
            actualAssign = IDuckSwapPool(pool).assignLimOrderEarnX(pt, amount, fromLegacy);
        }
    }
    ...
    function _updateOrder(
        ...
        if (order.sellXEarnY) {
            (, legacyAccEarn) = IDuckSwapPool(pool).decLimOrderWithX(order.pt, 0);
        } else {
            (, legacyAccEarn) = IDuckSwapPool(pool).decLimOrderWithY(order.pt, 0);
        }
        uint128 sold;
    ...
    function cancel(
        ...
        if (actualDecrease > 0) {
            if (sellXEarnY) {
                IDuckSwapPool(pool).decLimOrderWithX(order.pt, actualDecrease);
            } else {
                IDuckSwapPool(pool).decLimOrderWithY(order.pt, actualDecrease);
            }
        }
        if (actualDecrease > 0 || earn > 0) {
            IDuckSwapPool(pool).collectLimOrder(recipient, order.pt, actualDecrease, earn, sellXEarnY);
        }
    ...
    function collect(
        ...
        if (earn > 0) {
            IDuckSwapPool(pool).collectLimOrder(recipient, order.pt, 0, earn, sellXEarnY);
        }
    ...
    function swapDesireSingle(
        ...
        if (params.tokenOut < params.tokenIn) {
            ...
            (amountOut, amountIn) = IDuckSwapPool(poolAddr).swapY2XDesireX(
            ...
            // tokenOut is tokenY
            // tokenIn is tokenX
            (amountIn, amountOut) = IDuckSwapPool(poolAddr).swapX2YDesireY(
                recipient, params.amount, -799999,
                abi.encode(SwapCallbackData({
    ...
    function swapAmountSingle(
        ...
        if (params.tokenIn < params.tokenOut) {
            // swapX2Y
            (amountIn, amountOut) = IDuckSwapPool(poolAddr).swapX2Y(
                recipient, params.amount, -799999,
                abi.encode(SwapCallbackData({
                    tokenX: params.tokenIn, 
                    fee: params.fee, 
            ...
            // swapY2X
            (amountOut, amountIn) = IDuckSwapPool(poolAddr).swapY2X(
                recipient, params.amount, 799999,
                abi.encode(SwapCallbackData({
                    tokenX: params.tokenOut, 
                    fee: params.fee, 
