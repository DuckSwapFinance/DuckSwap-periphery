...
contract LimitOrderManager is Base, IDuckSwapAddLimOrderCallback {
    ...
    function decLimOrder(
        ...
        if (order.sellXEarnY) {
            (actualDeltaRefund, ) = IDuckSwapPool(pool).decLimOrderWithX(order.pt, actualDelta);
        } else {
            (actualDeltaRefund, ) = IDuckSwapPool(pool).decLimOrderWithY(order.pt, actualDelta);
        }
    ...
    function collectLimOrder(
        ...
        IDuckSwapPool(pool).collectLimOrder(recipient, order.pt, actualCollectDec, actualCollectEarn, order.sellXEarnY);
