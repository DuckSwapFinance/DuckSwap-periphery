...
contract UniversalSwapRouter is 
    ...
    constructor(address _duckSwapFactory, address _duckClassicFactory, address _weth, address _charger)
        UniversalBase(_weth)
        DuckSwapRouter(_duckSwapFactory)
        ClassicSwapRouter(_duckClassicFactory)
    {
        charger = _charger;
    }
    ...
    function pay(
        ...
        address recipient,
        uint256 value
    ) internal override(ClassicSwapRouter, DuckSwapRouter) {
    ...
    function swapDesireInternal(
    ...
    ) internal override(DuckSwapRouter, ClassicSwapRouter) returns (uint256 acquire, address tokenOut) {
        ...
        if (poolFee != 0) {
            // DuckSwap
            acquire = DuckSwapDesireSingleInternal(
                SwapSingleParams({
                    tokenIn: tokenIn,
                    tokenOut: tokenOut,
                    fee: poolFee,
    ...
    function swapAmountInternal(
        ...
        while (true) {
            ...
            if (poolFee != 0) {
                // DuckSwap
                (_cost, acquire) = DuckSwapAmountSingleInternal(
                    params,
                    payer
                );
            } else {
