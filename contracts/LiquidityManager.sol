...
contract LiquidityManager is Ownable, Base, ERC721Enumerable, IDuckSwapMintCallback {
    ...
    // callback data passed through DuckSwapPool#mint to the callback
    struct MintCallbackData {
        ...
        // fee amount of swap
        uint24 fee;
        // address to pay tokenX and tokenY to DuckSwapPool
        address payer;
    }
    ...
    constructor(
        address factory,
        address weth
    ) ERC721("DuckSwap Liquidity NFT", "DUCKSWAP-LIQUIDITY-NFT") Base(factory, weth) {
    }
    ...
    function createPool(address tokenX, address tokenY, uint24 fee, int24 initialPoint) external returns (address) {
        ...
        address pool = IDuckSwapFactory(factory).pool(tokenX, tokenY, fee);
        if (pool == address(0)) {
            pool = IDuckSwapFactory(factory).newPool(tokenX, tokenY, fee, initialPoint);
            return pool;
        }
        return pool;
    }
    ...
    function getLastFeeScale(address pool, bytes32 key) private view returns(uint256, uint256) {
        ...
        (, uint256 lastFeeScaleX_128, uint256 lastFeeScaleY_128, , ) = IDuckSwapPool(pool).liquidity(
            key
        );
    ...
    function _addLiquidity(MintParam memory mp) private returns(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        pool = IDuckSwapFactory(factory).pool(mp.tokenX, mp.tokenY, mp.fee);
        uint160 sqrtRate_96 = IDuckSwapPool(pool).sqrtRate_96();
        ...
        (amountX, amountY) = IDuckSwapPool(pool).mint(address(this), mp.pl, mp.pr, liquidity, 
    ...
    function addLiquidity(
        ...
        int24 currPt;
        uint160 sqrtPrice_96;
        address pool = IDuckSwapFactory(factory).pool(poolMeta.tokenX, poolMeta.tokenY, poolMeta.fee);
        // uint160 sqrtRate_96 = IDuckSwapPool(pool).sqrtRate_96();
        require(pool != address(0), "P0");
        (sqrtPrice_96, currPt) = getPoolPrice(pool);
        liquidityDelta = MintMath.computeLiquidity(
            ...
            sqrtPrice_96,
            // sqrtRate_96
            IDuckSwapPool(pool).sqrtRate_96()
        );
        require(int128(liquid.liquidity) == int256(uint256(liquid.liquidity)), "LO");
        uint128 newLiquidity = liquidityDelta + liquid.liquidity;
        (amountX, amountY) = IDuckSwapPool(pool).mint(address(this), liquid.leftPt, liquid.rightPt, liquidityDelta, 
    ...
    function decLiquidity(
        ...
        (amountX, amountY) = IDuckSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, liquidDelta);
    ...
    function collect(
        ...
        require(pool != address(0), "P0");
        if (liquidity.liquidity > 0) {
            IDuckSwapPool(pool).burn(liquidity.leftPt, liquidity.rightPt, 0);
        ...
        (amountX, amountY) = IDuckSwapPool(pool).collect(recipient, liquidity.leftPt, liquidity.rightPt, amountXLim, amountYLim);
